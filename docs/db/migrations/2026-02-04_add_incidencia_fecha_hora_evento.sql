-- 2026-02-04_add_incidencia_fecha_hora_evento.sql
-- Agrega fecha/hora real de ocurrencia para incidencias por dia de proyecto
-- Requiere haber aplicado: 2026-01-30_project_day_incidencias.sql

-- ==============================
-- 1) Columna de ocurrencia real
-- ==============================
SET @has_pdi_fecha_hora_evento := (
  SELECT COUNT(*)
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'T_ProyectoDiaIncidencia'
    AND COLUMN_NAME = 'PDI_FechaHora_Evento'
);

SET @sql_add_pdi_fecha_hora_evento := IF(
  @has_pdi_fecha_hora_evento = 0,
  'ALTER TABLE T_ProyectoDiaIncidencia ADD COLUMN PDI_FechaHora_Evento DATETIME NULL AFTER PDI_Descripcion',
  'SELECT 1'
);

PREPARE stmt_add_pdi_fecha_hora_evento FROM @sql_add_pdi_fecha_hora_evento;
EXECUTE stmt_add_pdi_fecha_hora_evento;
DEALLOCATE PREPARE stmt_add_pdi_fecha_hora_evento;

-- ===============================================
-- 2) SP: obtener proyecto (incluye fechaHoraEvento)
-- ===============================================
