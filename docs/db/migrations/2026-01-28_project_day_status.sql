-- 2026-01-28_project_day_status.sql
-- Estados por dia de proyecto (control operativo)

-- ==============================
-- 1) Tabla de estados por dia
-- ==============================
CREATE TABLE IF NOT EXISTS T_Estado_Proyecto_Dia (
...
INSERT IGNORE INTO T_Estado_Proyecto_Dia (PK_EPD_Cod, EPD_Nombre, EPD_Orden, Activo) VALUES
  (1, 'Pendiente', 1, 1),
  (2, 'En curso', 2, 1),
  (3, 'Terminado', 3, 1),
  (4, 'Suspendido', 4, 1),
...
-- ==============================
-- 2) Columna de estado en T_ProyectoDia
-- ==============================
ALTER TABLE T_ProyectoDia
  ADD COLUMN FK_EPD_Cod INT NOT NULL DEFAULT 1 AFTER FK_Pro_Cod,
  ADD KEY ix_proyecto_dia_estado (FK_EPD_Cod),
  ADD CONSTRAINT fk_proyecto_dia_estado
    FOREIGN KEY (FK_EPD_Cod) REFERENCES T_Estado_Proyecto_Dia (PK_EPD_Cod);

UPDATE T_ProyectoDia
SET FK_EPD_Cod = 1
WHERE FK_EPD_Cod IS NULL;

-- ==============================
-- 3) SP: obtener proyecto (agrega estado por dia)
-- ==============================
