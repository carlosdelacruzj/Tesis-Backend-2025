...
-- Database: defaultdb
-- Generated at: 2026-02-10T05:50:43.214Z
DELIMITER $$
...
BEGIN
  DECLARE v_total_raw DECIMAL(10,2) DEFAULT 0;  -- voucher (positivo)
  DECLARE v_total     DECIMAL(10,2) DEFAULT 0;  -- ABS(voucher)
  DECLARE v_pedidoId  INT DEFAULT NULL;

  DECLARE v_sumPedidoSinIgv DECIMAL(18,2) DEFAULT 0; -- SUM(PS_Subtotal)
  DECLARE v_viaticosSinIgv  DECIMAL(10,2) DEFAULT 0; -- P_ViaticosMonto
  ...
  /* 4) Totales SIN IGV: servicios + viáticos */
  ...
  IF v_basePedidoSinIgv <= 0 THEN
    -- fallback: asumimos que voucher incluye IGV, sacamos base
    SET v_factorPago_raw = 1;
    SET v_factorPago = 1;
    SET v_totalSinIgv = ROUND(v_total / 1.18, 2);
  ELSE
    -- ✅ factor considerando que voucher está CON IGV y base está SIN IGV
    ...
    -- ✅ total SIN IGV coherente con detalle usando factor redondeado
    SET v_totalSinIgv = ROUND(v_basePedidoSinIgv * v_factorPago, 2);
  END IF;
  ...
  /* ===== RESULTSET 2: DETALLE ===== */
  -- Servicios prorrateados + (opcional) línea de viáticos
  SELECT
    CAST(IFNULL(ps.PS_Cantidad,1) AS DECIMAL(10,2)) AS cantidad,
    'UNIDAD' AS unidad,
    CONCAT(
    ...
    ROUND(IFNULL(ps.PS_PrecioUnit,0) * v_factorPago, 2) AS valorUnitario,
    0.00 AS descuento,
    ROUND(ROUND(IFNULL(ps.PS_PrecioUnit,0) * v_factorPago, 2) * CAST(IFNULL(ps.PS_Cantidad,1) AS DECIMAL(10,2)), 2) AS importe
  FROM T_PedidoServicio ps
  WHERE ps.FK_P_Cod = v_pedidoId
  ...
  SELECT
    1.00 AS cantidad,
    'UNIDAD' AS unidad,
    'Viáticos' AS descripcion,
    ROUND(IFNULL(v_viaticosSinIgv,0) * v_factorPago, 2) AS valorUnitario,
    0.00 AS descuento,
    ROUND(IFNULL(v_viaticosSinIgv,0) * v_factorPago, 2) AS importe
...
-- PROCEDURE: SP_get_nota_credito_pdf_by_voucher
DROP PROCEDURE IF EXISTS `SP_get_nota_credito_pdf_by_voucher`$$
CREATE DEFINER="avnadmin"@"%" PROCEDURE "SP_get_nota_credito_pdf_by_voucher"(
  IN p_PK_Pa_Cod INT
)
BEGIN
  DECLARE v_pedidoId INT DEFAULT NULL;

  DECLARE v_total_raw DECIMAL(10,2) DEFAULT 0;     -- negativo
  DECLARE v_totalConIgv DECIMAL(10,2) DEFAULT 0;   -- ABS
  DECLARE v_totalSinIgv DECIMAL(10,2) DEFAULT 0;
  DECLARE v_igv DECIMAL(10,2) DEFAULT 0;
  ...
  DECLARE v_tipo VARCHAR(20) DEFAULT 'NC';
  DECLARE v_serie VARCHAR(10) DEFAULT 'NC01';
  DECLARE v_numero VARCHAR(20) DEFAULT '';

  DECLARE v_pedidoNombre VARCHAR(225) DEFAULT '';
  DECLARE v_pedidoFechaEvento DATE DEFAULT NULL;
  DECLARE v_pedidoLugar VARCHAR(150) DEFAULT '';

  -- comprobante afectado (último voucher positivo del mismo pedido)
  DECLARE v_voucherAfectadoId INT DEFAULT NULL;
  DECLARE v_tipoAfectado VARCHAR(20) DEFAULT '';
  DECLARE v_serieAfectado VARCHAR(10) DEFAULT '';
  DECLARE v_numeroAfectado VARCHAR(20) DEFAULT '';

  DECLARE v_motivo VARCHAR(255) DEFAULT 'Devolución';

  /* 1) Voucher NC -> Pedido + monto */
  SELECT v.FK_P_Cod, IFNULL(v.Pa_Monto_Depositado,0), IFNULL(v.Pa_Imagen_NombreOriginal,'')
    INTO v_pedidoId, v_total_raw, v_motivo
  FROM T_Voucher v
  WHERE v.PK_Pa_Cod = p_PK_Pa_Cod
  LIMIT 1;
  ...
  IF v_total_raw >= 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Este SP es solo para NOTA DE CREDITO (voucher negativo).';
  END IF;

  SET v_totalConIgv = ROUND(ABS(v_total_raw), 2);
  ...
  /* 4) Base e IGV de la devolución (total incluye IGV) */
  SET v_totalSinIgv = ROUND(v_totalConIgv / 1.18, 2);
  SET v_igv = ROUND(v_totalConIgv - v_totalSinIgv, 2);

  /* 5) Numeración NC (por ahora basada en el voucher NC) */
  SET v_numero = LPAD(p_PK_Pa_Cod, 8, '0');

  /* 6) Buscar comprobante afectado: último voucher positivo del pedido */
  SELECT v.PK_Pa_Cod
    INTO v_voucherAfectadoId
  FROM T_Voucher v
  WHERE v.FK_P_Cod = v_pedidoId
    AND v.Pa_Monto_Depositado > 0
  ORDER BY v.PK_Pa_Cod DESC
  LIMIT 1;

  IF v_voucherAfectadoId IS NOT NULL THEN
    IF v_clienteTipoDoc = 'RUC' THEN
      SET v_tipoAfectado = 'FACTURA';
      SET v_serieAfectado = 'F001';
    ELSE
      SET v_tipoAfectado = 'BOLETA';
      SET v_serieAfectado = 'B001';
    END IF;

    SET v_numeroAfectado = LPAD(v_voucherAfectadoId, 8, '0');
  ELSE
    SET v_tipoAfectado = IF(v_clienteTipoDoc='RUC','FACTURA','BOLETA');
    SET v_serieAfectado = IF(v_clienteTipoDoc='RUC','F001','B001');
    SET v_numeroAfectado = '00000000';
  END IF;
  ...
  SELECT
    ...
    v_pedidoId AS pedidoId,
    v_pedidoNombre AS pedidoNombre,
    v_pedidoFechaEvento AS pedidoFechaEvento,
    v_pedidoLugar AS pedidoLugar,
    ...
    0.00 AS anticipos,
    0.00 AS descuentoTotal,      -- ✅ para que no salga en blanco
    0.00 AS otrosCargos,
    0.00 AS otrosTributos,

    CONCAT(v_tipoAfectado, ' - Pedido ', v_pedidoId) AS docAfectadoTipo,
    CONCAT(v_serieAfectado, '-', v_numeroAfectado)   AS docAfectadoSerieNumero,
    IFNULL(NULLIF(TRIM(v_motivo),''), 'Devolución')   AS motivoNc;

  /* ===== RESULTSET 2: DETALLE ===== */
  SELECT
    1.00 AS cantidad,
    'UNIDAD' AS unidad,
    'Devolución' AS descripcion,
...
-- PROCEDURE: sp_nota_credito_devolucion_json
DROP PROCEDURE IF EXISTS `sp_nota_credito_devolucion_json`$$
CREATE DEFINER="avnadmin"@"%" PROCEDURE "sp_nota_credito_devolucion_json"(
  IN p_voucher_id INT,
...
BEGIN
  ...
  IF p_voucher_id IS NULL OR p_voucher_id <= 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'p_voucher_id invalido';
  END IF;

  IF (SELECT COUNT(*) FROM defaultdb.T_Voucher WHERE PK_Pa_Cod = p_voucher_id) = 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El voucher no existe';
  END IF;

  -- Solo vouchers aprobados
  IF (SELECT FK_EV_Cod FROM defaultdb.T_Voucher WHERE PK_Pa_Cod = p_voucher_id) <> 2 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El voucher no está aprobado';
  END IF;

  -- Total devuelto = ABS(monto negativo)
  SELECT ROUND(ABS(Pa_Monto_Depositado), 2)
    INTO v_total
  FROM defaultdb.T_Voucher
  WHERE PK_Pa_Cod = p_voucher_id;

  -- Desglose IGV (18%) desde total
  SET v_subtotal = ROUND(v_total / 1.18, 2);
  SET v_igv      = ROUND(v_total - v_subtotal, 2);

  -- JSON de salida (para DOCX / PDF)
  SELECT JSON_OBJECT(
    'tituloComprobante', 'NOTA DE CRÉDITO',
    'serie', 'NC01',

    -- Número del documento = ID del voucher
    'numero', CAST(v.PK_Pa_Cod AS CHAR),

    'fechaEmision', DATE_FORMAT(NOW(), '%Y-%m-%d'),
    'horaEmision',  DATE_FORMAT(NOW(), '%H:%i'),

    -- Documento afectado
    'docAfectadoTipo', 'VOUCHER',
    'docAfectadoSerieNumero', CONCAT('VCH-', v.PK_Pa_Cod),
    'pedidoCodigo', v.FK_P_Cod,

    'motivoNc', p_motivo,
    'moneda', '$',

    -- Info del voucher (opcional, pero útil)
    'voucher', JSON_OBJECT(
      'id', v.PK_Pa_Cod,
      'montoDepositado', v.Pa_Monto_Depositado,
      'metodoPagoId', v.FK_MP_Cod,
      'estadoVoucherId', v.FK_EV_Cod,
      'fechaVoucher', DATE_FORMAT(v.Pa_Fecha, '%Y-%m-%d')
    ),

    -- Detalle (1 línea por devolución)
    'detalleItems', JSON_ARRAY(
      JSON_OBJECT(
        'cantidad', 1,
        'unidad', 'NIU',
        'descripcion', CONCAT('Devolución (Pedido ', v.FK_P_Cod, ')'),
        'valorUnitario', v_subtotal,
        'descuento', 0,
        'importe', v_subtotal
      )
    ),
    ...
    'igv', v_igv,
    'total', v_total
  ) AS nota_credito_json
  FROM defaultdb.T_Voucher v
  WHERE v.PK_Pa_Cod = p_voucher_id;
