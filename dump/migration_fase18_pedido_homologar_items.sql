-- Fase 18: homologar campos de items entre cotizacion y pedido
-- Ejecuta este script una sola vez en la BD.

ALTER TABLE T_PedidoServicio
  ADD COLUMN PS_EventoId INT NULL AFTER FK_PE_Cod,
  ADD COLUMN PS_ServicioId INT NULL AFTER PS_EventoId,
  ADD COLUMN PS_Horas DECIMAL(4,1) NULL AFTER PS_Notas,
  ADD COLUMN PS_Staff SMALLINT NULL AFTER PS_Horas,
  ADD COLUMN PS_FotosImpresas INT NULL AFTER PS_Staff,
  ADD COLUMN PS_TrailerMin INT NULL AFTER PS_FotosImpresas,
  ADD COLUMN PS_FilmMin INT NULL AFTER PS_TrailerMin;
