...
BEGIN
  ...
  INSERT INTO defaultdb.T_Cotizacion(
    ...
    Cot_Lugar, Cot_HorasEst, Cot_Mensaje, Cot_Estado, FK_ECot_Cod
  )
  VALUES (
    v_lead_id,
    v_cli_id,
...
BEGIN
  ...
  SET Cot_Estado = p_estado_nuevo,
      FK_ECot_Cod = v_estado_nuevo_id
  WHERE PK_Cot_Cod = p_cot_id;

  -- devolver JSON de detalle (lead o cliente)
  SELECT JSON_OBJECT(
    'id',              c.PK_Cot_Cod,
    'estado',          COALESCE(ec.ECot_Nombre, c.Cot_Estado),
...
BEGIN
  SELECT
      ...
      COALESCE(ec.ECot_Nombre, c.Cot_Estado) AS estado,
